...
export function generateTableSvg(models: ModelInfo[]): string {
  ...
  const tableHeight = headerHeight + (models.length + 1) * cellHeight; // +1 for header row
...
`;
  ...
  svg += `
    <g class="header">
      ...
      <text x="${cellPadding}" y="${headerHeight / 2 + fontSize / 3}" dominant-baseline="middle">ID</text>
      <text x="${columnWidths[0] + cellPadding}" y="${headerHeight / 2 + fontSize / 3}" dominant-baseline="middle">Backend Model Name</text>
      <text x="${columnWidths[0] + columnWidths[1] + cellPadding}" y="${headerHeight / 2 + fontSize / 3}" dominant-baseline="middle">Front Name</text>
    </g>
  `;
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    svg += `
      <g class="${rowClass}">
        ...
        <text x="${cellPadding}" y="${y + cellHeight / 2 + fontSize / 3}" dominant-baseline="middle">${escapeXml(model.id)}</text>
        <text x="${columnWidths[0] + cellPadding}" y="${y + cellHeight / 2 + fontSize / 3}" dominant-baseline="middle" class="backend-name">${escapeXml(model.backendName)}</text>
        <text x="${columnWidths[0] + columnWidths[1] + cellPadding}" y="${y + cellHeight / 2 + fontSize / 3}" dominant-baseline="middle" class="front-name">${escapeXml(model.frontName)}</text>
      </g>
    `;
  }
